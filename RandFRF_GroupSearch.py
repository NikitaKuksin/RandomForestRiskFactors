...
class RandomFRF_Phenotype:
    ...
    def __init__(self,    
                          ...
                          subsample = None, colsample_bylevel = None, file_model = "", mls = 0
    ...
    def fit( self, x_data, y_data, models_pickles = "" ):
      ...
      rules = categorizer.getPointsOfInterest( x_data, y_data, grope_rule = True, models_pickles = models_pickles )
      ...
      for predict in range(0,self.pred_count):
        ...
        for rule in self.predictor_rules[predict]:
          ...
          if max_pred_prob[predict] <= self.predictor_rules[predict][rule]["Probability"][1]:
            max_pred_prob[predict] = self.predictor_rules[predict][rule]["Probability"][1]

        self.sum_max_prob = self.sum_max_prob + max_pred_prob[predict]

      logistReg = LogisticRegression()
      df_train_fist = df_train.drop( columns=df_train.columns[:self.pred_count] )

      logistReg.fit( df_train_fist.values, y_data )
      self.coef = logistReg.coef_[0]
      self.drop_columns = []
      self.df_train = df_train
      i = 0
      for predict in range(0,self.pred_count):
        rule_keys = copy(self.predictor_rules[predict])
        for rule_key in rule_keys:
          if self.coef[i] <= self.cutoff_risc_factor:
            self.predictor_rules[predict].pop( rule_key )
            df_train_fist = df_train_fist.drop( str(predict) + rule_key, axis = 1 )
          i = i + 1
    ...
    def predict_proba( self, x_test ):
      ...
      df_test["Risk_Faktors"] = None
      
      for index, patient in df_test.iterrows():
          df_test["Risk_Faktors"][index] = self.addRiskFactors( patient )

      probabilities = []
      for index, patient in df_test.iterrows():
        probability = self.FindeSimilar( patient )
        probabilities.append( [1-probability,probability] )  
      
      return probabilities
    
    def FindeSimilar( self, patient, min_patients = 10 ):
      
      min_patients = self.mls

      riskFaktors = pd.DataFrame(patient["Risk_Faktors"])
      if len( riskFaktors ) == 0:
        return 0
      
      riskFaktors = riskFaktors.sort_values(by="Importance", ascending=False)
      dataset = self.fit_dataset
      probobility = 0

      for index, rule in riskFaktors.iterrows():
        
        if not pd.isnull( rule["Min"] ) and not pd.isnull( rule["Max"] ):
          dataset_b = dataset.loc[ (( dataset[ dataset.columns[ int(rule["Predictor"]) ]] >= rule["Min"]) & (dataset.columns[ int(rule["Predictor"]) ] < rule["Max"])) ]
        elif not pd.isnull( rule["Min"] ):
          dataset_b = dataset.loc[ (( dataset[ dataset.columns[ int(rule["Predictor"]) ]] >= rule["Min"])) ]
        elif not pd.isnull( rule["Max"] ):
          dataset_b = dataset.loc[ (( dataset[ dataset.columns[ int(rule["Predictor"]) ]] < rule["Max"] )) ]

        len_dataset = len(dataset_b)
        if len_dataset < min_patients:
            #break
            continue
         
        probobility = sum(dataset_b["y"])/len_dataset
        dataset = dataset_b

      return probobility

    def addRiskFactors(self, patient):
      patients_risk_factors = []
      for predict in range(0,self.pred_count):
        for rule in self.predictor_rules[predict]:
          
          if ((self.predictor_rules[predict][rule]["Min"] == None or patient[predict] >= self.predictor_rules[predict][rule]["Min"]) and
              (self.predictor_rules[predict][rule]["Max"] == None or patient[predict] < self.predictor_rules[predict][rule]["Max"]) ):
            patients_risk_factors.append( {"Predictor":predict,
                                           "Min":self.predictor_rules[predict][rule]["Min"],
                                           "Max":self.predictor_rules[predict][rule]["Max"],
                                           "Importance":self.predictor_rules[predict][rule]["Importance"],} )
      return patients_risk_factors
...
#      model = RandomFRF_Prob(                   random_state=42, eval_metric="auc",
#                                                max_delta_step=3,
#                                                n_estimators = 225,
#                                                learning_rate = 0.025,
#                                                max_depth = 5,
