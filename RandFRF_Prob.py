    ...
    def predict_proba( self, x_test ):
      ...
      for predict in range(0,self.pred_count):
        for rule in self.predictor_rules[predict]:
          ...
          df_test.loc[ ( df_test[str(predict)+str(rule)] == min_val ), (str(predict)+str(rule)) ] = self.predictor_rules[predict][rule]["Probability"][1] 


      x_test = df_test.drop( columns=self.drop_columns ).values


      y_test_pred = self.get_prob(x_test)
